...
<img src="http://latex.codecogs.com/gif.latex?SS_{Total}=SS_{Treatment}+SS_{Error}\Rightarrow\\\\\sum_{i=1}^{a}\sum_{j=1}^{n}(y_{ij}-\bar{y_{..}})^{2}=\sum_{i=1}^{a}\sum_{j=1}^{n}(\bar{y_{i.}}-\bar{y_{..}})^{2}+\sum_{i=1}^{a}\sum_{j=1}^{n}(y_{ij}-\bar{y_{i.}})^{2}" border="0"/>. Since there are **a** treatments and **n** replicates, there are total of **N** observations. As a result the sum of squares of total has <img src="http://latex.codecogs.com/gif.latex?N-1" border="0"/> degrees of freedom, <img src="http://latex.codecogs.com/gif.latex?SS_{Treatment}" border="0"/> has <img src="http://latex.codecogs.com/gif.latex?a-1" border="0"/>, and <img src="http://latex.codecogs.com/gif.latex?SS_{Error}" border="0"/> has <img src="http://latex.codecogs.com/gif.latex?N-a" border="0"/> degrees of freedom.

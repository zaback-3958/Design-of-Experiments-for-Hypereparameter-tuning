...
Sometimes even half fraction design is very large because of the numebr of factors, and it needs a large number of runs. in this case the fraction of the design can be increased from a half fraction to a quarter fraction as  <img src="http://latex.codecogs.com/svg.latex?2^{K-2}=\frac{1}{2}\times(2^{K-1})"/>  which is the half fraction of the previous fractional design.In this design at first half of the design with its generator(s) is selected and then from the first half the second half with its generator(s) will be selected. 

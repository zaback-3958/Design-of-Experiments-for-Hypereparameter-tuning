...
fmeasure.func <- function(
    ...
    print( recal.train );
  	...
  	precs.train = precision.fnc(DF.input = recal.train)
  	precs.valid = precision.fnc(DF.input = recal.valid)
  	precs.test  = precision.fnc(DF.input = recal.test)

  	cat("\n print(precs.train)\n");
    print( precs.train );
  
  	fmeasure.train = fmeasure.fnc(DF.recal = recal.train,DF.precision = precs.train)
  	fmeasure.valid = fmeasure.fnc(DF.recal = recal.valid,DF.precision = precs.valid)
  	fmeasure.test  = fmeasure.fnc(DF.recal = recal.test,DF.precision = precs.test)
  
  	cat("\n print(fmeasure.train)\n");
    print( fmeasure.train );
    ...
    return( list(
	    DF.fmtrain = fmeasure.train,
	    DF.fmvalid = fmeasure.valid,
	    DF.fmtest  = fmeasure.test));
...
####################################################
	confusion.mtrx <- function(
		DF.input = NULL,
		classes  = NULL){
			TP = DF.input[classes[2], classes[2]];
			FN = DF.input[classes[2], classes[1]];
			FP = DF.input[classes[1], classes[2]];
			TN = DF.input[classes[1], classes[1]]
